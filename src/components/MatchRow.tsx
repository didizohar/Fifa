...
export function MatchRow({ matchType, isPenalties, side1, side2, playedAtLabel, onPress }: MatchRowProps) {
  ...
  const styles = useMemo(
    () => ({
      card: { backgroundColor: colors.surface, borderRadius: radius.lg, borderWidth: 1, borderColor: colors.borderSubtle, padding: spacing.md, gap: spacing.sm, ...shadows.sm },
      pressed: { opacity: 0.8 },
      header: { flexDirection: "row" as const, alignItems: "center" as const, gap: spacing.sm },
      date: { ...typography.small, marginStart: "auto" as const },
      sideRow: { flexDirection: "row" as const, alignItems: "center" as const, gap: spacing.sm },
      sideInfo: { flex: 1, gap: 2 },
      sideLabel: { ...typography.bodyStrong },
      sideLabelMuted: { color: colors.textSecondary },
      clubName: { ...typography.small },
      score: { ...typography.heading, minWidth: 26 },
  ...
  return (
    ...
    >
      ...
      <SideRow side={side1} resultLabel={resultLabel} resultColor={resultColor} styles={styles} />
      <SideRow side={side2} resultLabel={resultLabel} resultColor={resultColor} styles={styles} />
    </Pressable>
  );
}
...
function SideRow({
  ...
  side: MatchRowSide;
  resultLabel: Record<MatchRowSide["result"], string>;
  resultColor: Record<MatchRowSide["result"], string>;
  styles: Record<string, object>;
}) {
  const { isRTL } = useTranslation();
  return (
    <View style={styles.sideRow}>
      <ClubCrest logoUrl={side.clubLogoUrl} size={28} />
      ...
      <Text
        style={[
          styles.score,
          { color: resultColor[side.result], textAlign: isRTL ? "left" : "right" },
